...
const catalog = JSON.parse(await readFile(path.join(producer, 'catalog.json'), 'utf8')) as {
	slug: string;
	category: string;
}[];
...
test('every catalog entry has a downloadable detail page, poster and gallery preview', async () => {
	...
	for (const {slug} of catalog) {
		...
		assert.ok(overview.includes(`/posters/${slug}.webp`), `${slug}: missing gallery poster`);
		...
		assert.ok(overview.includes(`/previews/${slug}.webm`), `${slug}: missing preview`);
	...
	assert.ok(!/<video[^>]*autoplay/.test(overview), 'Gallery must not force autoplay');
});
...
test('categories: gallery and sidebar group all elements without changing their URLs', async () => {
	...
	for (const category of ['visualizers', 'shaders', 'effects']) {
		...
		for (const entry of catalog) {
			assert.equal(section.includes(`href="/${entry.slug}.html"`), entry.category === category);
		}
	}
